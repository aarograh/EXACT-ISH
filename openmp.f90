...
!===============================================================================
    !This version vectorize the third block
    !actually vectorize all blocks and inline the exponetial table lookup.
...
!     !Based on the previous performances.
!     !This version tried to include omp
!     SUBROUTINE sweep2D_prodquad_P0_vectoripol3_4(sweeper,i)
!       CLASS(sweeperType),INTENT(INOUT) :: sweeper
!       INTEGER,INTENT(IN) :: i
...
! !$OMP PARALLEL NUM_THREADS(nproc) DEFAULT(PRIVATE) &
! !$OMP SHARED(sweeper,npol,wsum,ifrstreg_proc,expoa,expob,PI)
! !$    ithd=OMP_GET_THREAD_NUM()+1
!       ALLOCATE(phid1(npol))
!       ALLOCATE(phid2(npol))
!       ALLOCATE(phio1(npol,0:sweeper%maxsegray))
...
!         sweeper%qbar*wsum
!       DEALLOCATE(tphi)
!     ENDSUBROUTINE sweep2D_prodquad_P0_vectoripol3_4
